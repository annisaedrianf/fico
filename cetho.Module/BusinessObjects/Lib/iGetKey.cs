...
namespace cetho.Module.BusinessObjects
{
    ...
    public class iGetKey
    {
        ...
        public Tuple<bool, bool> isBool(string oBool)
        {
            ...
            if (bool.TryParse(oBool, out bValue))
            {
                bBool = true;
            }
            else
        ...
        public bool IsAnyDataSync(List<InfoOfClass> olInfoOfClass)
        {
            bool isAnyData = false;
            ...
            try
            {
                ...
                if (query != null)
                {
                    query = olInfoOfClass.Where(a => a.isKeySync && a.Value.ToString() == "").ToList();
                    if (query == null)
                    {
                        isAnyData = true;
                    }
                    ...
                    {
                        query = olInfoOfClass.Where(a => a.isKeySync).ToList();
                        query = query.Where(a => a.Value.ToString() != "").ToList();
                        if (query.Count > 0)
                        {
                            isAnyData = true;
        ...
        public List<Type> GetClassList()
        {
            ...
            {
                if (type.Namespace == "Nucleus.Module.BusinessObject")
                {
                    types.Add(type);
                }
            }
        ...
        public List<InfoOfClass> GetObjProperty<T>(T oType, Session session, XPCollection<SyncObjectColom> oXPColom) where T : Type
        {
            ...
            foreach (XPMemberInfo info in classInfo.PersistentProperties)
            {
                ...
                if (!aNotlist.Contains(fieldName.ToUpper()))
                {
                    ...
                    List<SyncObjectColom> loXPColom = oXPColom.Where(a => a.ImportName != null && a.ImportName.ToString() != "").ToList();
                    int ColomIdx = -1;
                    foreach (SyncObjectColom icolName in loXPColom)
                    {
                        if (icolName.ObjectName.ToLower() == oInfoOfClass.Name.ToLower())
                        {
                            //ColomIdx = ColomIdx + 1;
                            //oInfoOfClass.ColomXls = icolName.ImportName.ColomIdx; di remarks 
                            // set bahwa kolom ter mapping
                            oInfoOfClass.ColomXls = icolName.ImportNameIdx;
                            ...
                            foreach (Attribute ItemAtribut in listOfAttribut)
                            {
                                ...
                                if (ItemAtribut is IsSyncSearch)
                                {
                                    IsSyncSearch attributeObject = (IsSyncSearch)ItemAtribut;
                                    oInfoOfClass.isKeySync = attributeObject.isSyncSearch;
                                    oInfoOfClass.allKeySync = attributeObject.allKey;
                                }
        ...
        public List<InfoOfClass>  GetObjProperty<T>(T oType, Session session, Worksheet worksheet) where T : Type
        {
            ...
            foreach (XPMemberInfo info in classInfo.PersistentProperties)
            {
                ...
                //foreach (Attribute at in info.Attributes)
                //{
                //    if (  at is IsHide)
                //    {
                //        aNotlist.Add(fieldName);
                //    }
                //}
                // check the name is not default system name
